...
ggplot(avg_Temp_bootstrap, aes(x = stat)) +
  geom_histogram(bins = 30, fill = "green", color = "darkgreen") +
  theme_bw() +
  labs(title = "Bootstrap distribution of Average Temperature", x = "Mean Temperature (C°)", y = "Frequency")
```

This histogram shows the distribution of average temperatures from 1000 samples using the bootstraping method. This shows what we expect the average temperature to be if we randomly select a sample from our population.
...
This shows the 95% confidence interval of the bootstraped distribution. We are 95% confident that the mean of a sample of the Average Tempurature will be between approximately 6.8 and 8.7 degrees Celsius. 
<br>
